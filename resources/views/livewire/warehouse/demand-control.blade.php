<div class="p-6">
    <div class="mb-6">
        ...
        <p class="text-zinc-500 dark:text-zinc-400">Control point for all production and purchase authorizations.</p>
    </div>
    ...
    <div class="tabs tabs-boxed mb-6 bg-zinc-100 dark:bg-zinc-800 p-1">
        <a wire:click="$set('activeTab', 'fg')" class="tab {{ $activeTab === 'fg' ? 'tab-active' : '' }}">Finished Goods Demands</a>
        <a wire:click="$set('activeTab', 'rm')" class="tab {{ $activeTab === 'rm' ? 'tab-active' : '' }}">Raw Material Requisitions</a>
    </div>
            ...
            <div class="bg-white dark:bg-zinc-900 rounded-xl border border-zinc-200 dark:border-zinc-800 overflow-hidden shadow-sm">
                ...
                <div class="overflow-x-auto">
                    <table class="table table-zebra w-full">
                        ...
                        <tbody>
                            @foreach ($selectedOrderFgDemands as $demand)
                                <tr>
                                    <td>
                        ...
                        <tbody>
                            @forelse ($ordersWithStockDemands as $order)
                                <tr>
                                    ...
                                    <td>
                                        <span class="badge badge-info">{{ $order->orderItems()->whereHas('stockDemands', function($q){$q->where('status','pending');})->count() }} items</span>
                                    </td>
                                    <td>
    ...
    @else
        ...
        <div class="bg-white dark:bg-zinc-900 rounded-xl border border-zinc-200 dark:border-zinc-800 overflow-hidden shadow-sm">
            <div class="p-4 border-b border-zinc-200 dark:border-zinc-800 flex justify-between items-center">
                <h3 class="font-bold">Material Requisitions Awaiting Approval</h3>
                    ...
                    <tbody>
                        @forelse ($rmDemands as $req)
                            <tr>
                                ...
                                <td>
                                    <span class="badge badge-ghost">{{ $req->rawMaterial->quantity }} {{ $req->rawMaterial->unit }}</span>
                                </td>
                                <td>
