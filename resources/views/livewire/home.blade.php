<div class="space-y-6 p-6">
    ...
    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <div class="rounded-lg border border-gray-200 bg-white p-4 dark:border-gray-700 dark:bg-gray-800">
            ...
            <p class="mt-1 text-2xl font-semibold text-gray-900 dark:text-white">{{ number_format($salesStats['total_revenue'] ?? 0, 2) }}</p>
        </div>

        <div class="rounded-lg border border-gray-200 bg-white p-4 dark:border-gray-700 dark:bg-gray-800">
            <p class="text-sm text-gray-600 dark:text-gray-400">Total Orders</p>
            <p class="mt-1 text-2xl font-semibold text-gray-900 dark:text-white">{{ number_format($salesStats['total_orders'] ?? 0) }}</p>
        </div>

        <div class="rounded-lg border border-gray-200 bg-white p-4 dark:border-gray-700 dark:bg-gray-800">
            <p class="text-sm text-gray-600 dark:text-gray-400">Pending Orders</p>
            <p class="mt-1 text-2xl font-semibold text-gray-900 dark:text-white">{{ number_format($salesStats['pending_orders'] ?? 0) }}</p>
        </div>

        <div class="rounded-lg border border-gray-200 bg-white p-4 dark:border-gray-700 dark:bg-gray-800">
            <p class="text-sm text-gray-600 dark:text-gray-400">Order Completion</p>
            <p class="mt-1 text-2xl font-semibold text-gray-900 dark:text-white">{{ number_format($salesStats['completion_rate'] ?? 0, 1) }}%</p>
        </div>
    </div>
        ...
        <div class="rounded-lg border border-gray-200 bg-white p-6 dark:border-gray-700 dark:bg-gray-800">
            ...
            <div class="space-y-3">
                <div class="flex items-center justify-between">
                    ...
                    <span class="font-semibold text-gray-900 dark:text-white">{{ $salesStats['pending_orders'] ?? 0 }}</span>
                </div>
                <div class="flex items-center justify-between">
                    <span class="text-sm text-gray-700 dark:text-gray-300">In Production</span>
                    <span class="font-semibold text-gray-900 dark:text-white">{{ $salesStats['in_production'] ?? 0 }}</span>
                </div>
                <div class="flex items-center justify-between">
                    <span class="text-sm text-gray-700 dark:text-gray-300">Completed</span>
                    <span class="font-semibold text-gray-900 dark:text-white">{{ $salesStats['completed'] ?? 0 }}</span>
                </div>
                <div class="flex items-center justify-between">
                    <span class="text-sm text-gray-700 dark:text-gray-300">Delivered</span>
                    <span class="font-semibold text-gray-900 dark:text-white">{{ $salesStats['delivered'] ?? 0 }}</span>
                </div>
            </div>
        </div>
    </div>
