...
@auth
    <nav class="mobile-bottom-nav">
        <!-- Common: Dashboard (visible to all) -->
        @can('dashboard.view')
        ...
        @canany(['warehouse.dashboard', 'warehouse.stock-overview', 'warehouse.pending-receipts'])
        ...
        <!-- Finance Department -->
        @canany(['finance.dashboard', 'finance.procurement', 'finance.purchase-payments'])
        ...
        @canany(['operations.dashboard', 'operations.production-orders', 'operations.demand-control'])
            <a href="{{ route('operations.index') }}" class="{{ request()->routeIs('operations.*') ? 'active' : '' }}">
                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <!-- Common: Profile (visible to all) -->
        <a href="/settings/profile" class="{{ request()->routeIs('settings.*') ? 'active' : '' }}">
            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
